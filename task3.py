...
def morph_erode(img):
    ...
    """
    img_bool = img == 255
    st_bool = np.ones((3, 3), dtype=bool)
    new_img = np.zeros_like(img_bool, dtype=bool)
    padded_img = np.zeros((img.shape[0] + 2, img.shape[1] + 2), dtype=bool)
    padded_img[1:img.shape[0] + 1, 1:img.shape[1] + 1] = img_bool
    for i in range(1, img_bool.shape[0]):
        for j in range(1, img_bool.shape[1]):
            new_img[i, j] = (padded_img[i:i + 3, j:j + 3] == st_bool).all()
    erode_img = np.where(new_img == True, 255, 0)
    return erode_img
...
def morph_dilate(img):
    ...
    for i in range(1, img_bool.shape[0]):
        for j in range(1, img_bool.shape[1]):
            new_img[i, j] = (padded_img[i:i + 3, j:j + 3] == st_bool).any()
    dilate_img = np.where(new_img == True, 255, 0)
    return dilate_img
...
def morph_open(img):
    ...
    erosed_img = morph_erode(img)
    open_img = morph_dilate(erosed_img)
    return open_img
...
def boundary(img):
    ...
    # TO DO: implement your solution here
    bound_img = morph_erode(img)
    bound_img = img - bound_img
    bound_img = bound_img.astype(int)
    return bound_img
